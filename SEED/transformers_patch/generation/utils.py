...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # print('generation config: ', kwargs)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # model_kwargs['sub_batch_beam_size'] = 256
            # print('cur_len: ', cur_len)

            # if 'sub_batch_beam_size' in model_kwargs:
            #     sub_batch_beam_size = model_kwargs['sub_batch_beam_size']
            #     # split model_inputs into chunks
            #     num_sub_batches = math.ceil(batch_beam_size / sub_batch_beam_size)
            #     # print(batch_beam_size, sub_batch_beam_size, num_sub_batches)
            #     model_inputs_sub = [type(model_inputs)() for _ in range(num_sub_batches)]
            #     for k, v in model_inputs.items():
            #         if v is None or isinstance(v, bool):
            #             for i_sub in range(num_sub_batches):
            #                 model_inputs_sub[i_sub][k] = v
            #         elif isinstance(v, torch.Tensor):
            #             v_sub = torch.split(v, sub_batch_beam_size, dim=0)
            #             assert len(v_sub) == num_sub_batches, f'{len(v_sub)}, {num_sub_batches}'
            #             for i_sub in range(num_sub_batches):
            #                 model_inputs_sub[i_sub][k] = v_sub[i_sub]
            #         else:
            #             raise ValueError(f'Unknown type of v: {type(v)}, value: {v}, key: {k}')
            #     # forward
            #     outputs_sub_batches = []
            #     for i_sub in range(num_sub_batches):
            #         # print('i_sub: ', i_sub)
            #         with torch.no_grad():
            #             outputs = self(
            #                 **model_inputs_sub[i_sub],
            #                 return_dict=True,
            #                 output_attentions=output_attentions,
            #                 output_hidden_states=output_hidden_states,
            #             )
            #             outputs_sub_batches.append(outputs)
            #         torch.cuda.empty_cache()
            #     # merge 
            #     outputs = type(outputs_sub_batches[0])()
            #     for k, v in outputs_sub_batches[0].items():
            #         if v is None or isinstance(v, bool):
            #             outputs[k] = v
            #         elif isinstance(v, torch.Tensor):
            #             outputs[k] = torch.cat([outputs_sub[k] for outputs_sub in outputs_sub_batches], dim=0)
            #         else:
            #             raise ValueError(f'Unknown type of v: {type(v)}, value: {v}, key: {k}')
