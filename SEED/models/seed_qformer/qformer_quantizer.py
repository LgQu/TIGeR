...
class Blip2QformerQuantizer(Blip2Base):
    ...
    def get_codebook_indices(self, image):
        with torch.no_grad():
            ...
            # print('query_output_down:',query_output_down.shape)
            ...
            # print('query_output_up:',query_output_up.shape)
        return embed_ind, query_output_up

    def get_codebook_entry(self, indices):
        quant_embedding = self.quantize.get_codebook_entry(indices) # [32]
        # print('quant_embedding_shape: ', quant_embedding.shape)
        # print(self.decode_task_layer)
        # exit()
        query_output_up = self.decode_task_layer(quant_embedding) # [768]
        # print('query_output_up:', query_output_up.shape)
        ...
        # print('query_output_up_pos:', query_output_up.shape)
        if self.use_qformer_image:
            ...
            # print('reverse_output:', reverse_output.shape)
            reverse_output_proj = self.distill_image_proj(reverse_output).squeeze(1)
            # print('reverse_output_proj:', reverse_output_proj.shape)
        else:
            reverse_output = self.image_down(query_output_up) # [768, 32, 32]
            # print('reverse_output(bef reshape):', reverse_output.shape)
            reverse_output = reverse_output.reshape(reverse_output.shape[0], -1) # [768,1024]
            # print('reverse_output(aft reshape):', reverse_output.shape)
            reverse_output_proj = self.distill_image_proj(reverse_output) # [768,1024]
            # print('reverse_output_proj:', reverse_output_proj.shape)
