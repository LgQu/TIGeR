...
class StableUnCLIPImg2ImgPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def _encode_image(
        ...
        # print("repeat by:", repeat_by)
        # print("image_embeds(bef trans):", image_embeds.shape)
        # duplicate image embeddings for each generation per prompt, using mps friendly method
        image_embeds = image_embeds.unsqueeze(1)
        bs_embed, seq_len, _ = image_embeds.shape
        # print("image_embeds(bef repeat):", image_embeds.shape)
        image_embeds = image_embeds.repeat(1, repeat_by, 1)
        # print("image_embeds(aft repeat):", image_embeds.shape)
        image_embeds = image_embeds.view(bs_embed * repeat_by, seq_len, -1)
        image_embeds = image_embeds.squeeze(1)
        # print("image_embeds(aft trans):", image_embeds.shape)
        ...
        # print("image embeds(bef cfg cat):", image_embeds.shape)
        if do_classifier_free_guidance:
        ...
        # print("image embeds(aft cfg cat):", image_embeds.shape)
        return image_embeds
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        ...
        # print("latents:", latents, latents.shape, shape)
        if latents is None:
    ...
    def __call__(
        ...
        # print("len(image):", image, prompt, prompt_embeds)
        if prompt is None and prompt_embeds is None:
            if image:
                prompt = len(image) * [""] if isinstance(image, list) else ""
            else:
                prompt = len(image_embeds) * [""]
        ...
        # print("image embed:", image_embeds.shape)
        # 5. Prepare timesteps
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            # print("t:", t.shape, "latents:", latents.shape, "do cfg:", do_classifier_free_guidance)
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            # print("latent_model_input(bef scale):", latent_model_input.shape)
            latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
            # print("latent_model_input(aft scale):", latent_model_input.shape)
