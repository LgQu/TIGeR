...
class ImageTokenizer(nn.Module):
    def __init__(self,
        ...
        if diffusion_model_path is not None and load_diffusion:
            ...
            diffusion_model = StableUnCLIPImg2ImgPipeline.from_pretrained(diffusion_model_path, cache_dir=os.getenv("HF_HOME"),
                                                                          torch_dtype=torch.float16 if fp16 else torch.float32)
    ...
    def decode(self, indices, negative_indices=None, guidance_scale=10, num_inference_steps=20):
        ...
        # self.latents = self.latents.expand(batch,-1,-1,-1)
...
class SeedLlamaTokenizer(LlamaTokenizer):
    ...
    def decode_image(self, indices, negative_indices=None, guidance_scale=10, num_inference_steps=20):
