...
from models.modeling_visual_tokenizer import build_dynamic_tokenizer, VectorQuantizer
...
class LaVITforGeneration(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.llama_tokenizer.padding_side = "left" # left padding
        ...
        if load_pixel_decoder:
            ...
            self.vae = AutoencoderKL.from_pretrained(diff_model_dir, subfolder="vae",
                    torch_dtype=torch.bfloat16 if model_dtype=="bf16" else torch.float16).to('cuda')
    ...
    def generate_image_embeds(self, image_tokens):
        ...
        token_nums = torch.as_tensor(token_nums, dtype=torch.long).to('cuda')
        ...
        # token_nums.to('cuda')
        # print("token_nums:", token_nums.device, "token_quantize:", token_quantize.device)
    ...
    def generate_image(self, prompts, 
        ...
        if self.pixel_decoding == 'lowres':
            output_images = self.pixel_decoding_origin(image_tokens, width, height, num_inference_steps, guidance_scale_for_decoder)
            if return_image_ids:
                return image_tokens, output_images
            else:
                return output_images
    ...
    @torch.no_grad()
    def get_selected_image_ids(self, prompts, 
        width=512,
        height=512,
        original_size=None,
        crops_coords_top_left=None,
        ...
        for prompt_str, prompt_type in prompts:
            ...
            if prompt_type == 'text':
                ...
                input_prompts.append(text_tokens)
            if prompt_type == 'image':
                if isinstance(prompt_str, Image.Image):
                    image_input = prompt_str.convert("RGB")
                else:
                    image_input = Image.open(prompt_str).convert("RGB")
                image_tensor = self.processer(image_input).unsqueeze(0).to(device)
                with self.maybe_autocast():
                    image_tokens, pred_scores = self.visual_tokenizer.tokenize_image(image_tensor, add_special=False, select_topk=select_topk, 
                                select_interval=select_interval, select_random=select_random, select_threshold=select_threshold)

        # input_prompts = [torch.cat(input_prompts, dim=0)]
        # print(input_prompts, pred_scores)
        assert len(image_tokens) == 1
        return image_tokens[0], pred_scores
