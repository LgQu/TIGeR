...
class DynamicVisualTokenizer(nn.Module):
    ...
    def encode_features(self, x):
        ...
        """
        device = x.device
        # print('encoder---', self.encoder.patch_embed.proj.weight.device, x.device)
    ...
    def tokenize_image(self, x_tensor, add_special=False):
        ...
        pred_score = self.token_predictor(encoder_features.to(torch.float32), mask).reshape(B, -1, 2)
        print("pred_score:", pred_score, pred_score.shape) # [N, 256, 2]
        # Sample from the score distribution
        hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0]   # [N, num_patches]
        ...
        B, N, C = updated_features.shape
        index_select = hard_keep_decision.long()
        # print("index_select:", index_select.shape) # [N, 256]
        token_nums = index_select.sum(dim=-1) # selected tokens x T
        # print("select token nums:", token_nums)
        index_select = index_select.bool()
        ...
        print("embed_ind_list:", embed_ind_list, len(embed_ind_list))
        return embed_ind_list

    def tokenize_image_legacy(self, x_tensor, add_special=False, select_all=False):
        # x_tensor: [bs, 3, h, w]
        feature_targets = self.encoder(x_tensor, return_all_features=True)   # N, 257, D
        encoder_features = feature_targets[:, 1:, :]  # Exclude the class token.
        ...
        # Sample from the score distribution
        if select_all:
            assert False, 'Cross attention module will get all-zero attention values, which is wrong!'
            hard_keep_decision = torch.ones(pred_score.shape[0], pred_score.shape[1], device=pred_score.device)   # [N, num_patches]
        else:
            hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0]   # [N, num_patches]

        # Update the existed features from dropped tokens (To remain the information flow)
        updated_features = self.causal_encoder(encoder_features, hard_keep_decision)
        # print("updated_feautre:", updated_features, updated_features.shape)
        B, N, C = updated_features.shape
        index_select = hard_keep_decision.long()
        # print("index_select:", index_select.shape) # [N, 256]
        token_nums = index_select.sum(dim=-1) # selected tokens x T
        ...
        index_select = index_select.bool()
        remained_token = torch.masked_select(updated_features, index_select[:,:,None]).reshape(-1, C)  # [Num Patch]
        remained_pred_scores = torch.masked_select(pred_score, index_select[:,:,None]).reshape(-1, 2)  # [Num Patch]
        # print(remained_pred_scores, remained_pred_scores.shape)
        to_quantizer_features = self.encode_task_layer(remained_token.type_as(self.encode_task_layer[-1].weight))  
        quantize, embed_ind = self.quantize.tokenize(to_quantizer_features)
        # print("embed_ind:", embed_ind.shape)
        embed_ind = embed_ind + 32002
        embed_ind_list = torch.split(embed_ind, token_nums.tolist(), dim=0)
        # pred_scores_list = torch.split(pred_score, token_nums.tolist(), dim=1)
        ...
        if add_special:
            ...
            for ele in embed_ind_list:
                output_embed_ind.append(torch.cat([image_special[:1], ele, image_special[1:]]))
            return output_embed_ind, remained_pred_scores

        return embed_ind_list, remained_pred_scores

    def tokenize_image(self, x_tensor, add_special=False, select_all=False, select_topk=None, select_interval=None, 
                            select_random=None, select_threshold=None):
        # x_tensor: [bs, 3, h, w]
        feature_targets = self.encoder(x_tensor, return_all_features=True)   # N, 257, D
        encoder_features = feature_targets[:, 1:, :]  # Exclude the class token.
        ...
        if select_all:
            assert False, 'Cross attention module will get all-zero attention values, which is wrong!'
            hard_keep_decision = torch.ones(pred_score.shape[0], pred_score.shape[1], device=pred_score.device)   # [N, num_patches]
        elif select_topk is not None: 
            s = F.softmax(pred_score, dim=-1)[:, :, 0] # [N, num_patches]
            topk_idx = s.topk(select_topk, dim=-1).indices
            hard_keep_decision = torch.zeros_like(s)
            hard_keep_decision.scatter_(index=topk_idx, dim=-1, value=1)
        elif select_interval is not None:
            hard_keep_decision = torch.zeros(pred_score.shape[0], pred_score.shape[1], device=pred_score.device)   # [N, num_patches]
            hard_keep_decision[:, ::select_interval] = 1
        elif select_random is not None:
            random_idx = []
            for i in range(pred_score.shape[0]):
                idx = torch.randperm(pred_score.shape[1])[:select_random]
                random_idx.append(idx)
            random_idx = torch.stack(random_idx).to(pred_score.device)
            hard_keep_decision = torch.zeros(pred_score.shape[0], pred_score.shape[1], device=pred_score.device)   # [N, num_patches]
            hard_keep_decision.scatter_(index=random_idx, dim=-1, value=1)
        elif select_threshold is not None:
            s = F.softmax(pred_score, dim=-1)[:, :, 0] # [N, num_patches]
            hard_keep_decision = (s > select_threshold).to(dtype=pred_score.dtype, device=pred_score.device)
        else:
            hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0]   # [N, num_patches]
