...
def get_txt_tokens(all_text, clip_version):
    all_text_tokens = None
    if clip_version == 'clip':
        all_text_tokens = clip.tokenize(all_text, context_length=77, truncate=True)
    elif clip_version == 'openclip':
        all_text_tokens = open_clip.tokenizer.tokenize(all_text, context_length=77)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ann_root", default=None)
    ...
    parser.add_argument('--clip_version', default='openclip', choices=['clip', 'openclip'])
    ...
    if args.use_gt:
        ...
        ranking[sum(args.nums_selected_txt_query[:2]):, 0] = gt_id
    ...
    if args.post_filtering_mask is not None:
        ...
        assert len(ranking) == len(all_img_path_gen) == filtering_mask.sum()
    ...
    sampled_text_tokens = get_txt_tokens(sampled_text, args.clip_version)
    ...
    print('Load model ...')
    if args.clip_version == 'clip':
        model, preprocess = clip.load("ViT-L/14")
    elif args.clip_version == 'openclip':
...
workbook, worksheet, style0, style1 = create_worksheet()
